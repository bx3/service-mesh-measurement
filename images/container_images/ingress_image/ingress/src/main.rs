...
fn main() {
    ...
    let wordcount_socket = args[2].clone();
    let reverse_socket = args[3].clone();

    let server = Server::http(server_socket).unwrap();

    for mut request in server.incoming_requests() {
        let wc_sock: String = wordcount_socket.clone();
        let re_sock: String = reverse_socket.clone();
        ...
        let _ = thread::spawn(move || {
            match request.url() {
                "/run" => {
                    ...
                    let wordcount_result = match query_service(&map, &wc_sock, "wordcount") {
                        Ok(result) => result,
                        Err(e) => {
                    ...
                    println!("run reverse");
                    let reverse_result = match query_service(&map, &re_sock, "reverse") {
                        Ok(r) => r,
                        Err(e) => {
                ...
                "/stop" => {
                    ...
                    let wordcount_result = query_service(&HashMap::new(), &wc_sock, "stop").unwrap();
                    let reverse_result = query_service(&HashMap::new(), &re_sock, "stop").unwrap();
